...
import com.hcbs.service.CancellationService;
...
public class CancellationView extends VerticalLayout {
    ...
    private void findBooking() {
        try {
            Booking booking = cancellationService.findBookingByReference(reference.getValue());
            details.setValue(format(booking));
    ...
    private void cancelBooking() {
        try {
            Booking booking = cancellationService.cancelBooking(reference.getValue());
            details.setValue(format(booking));
        } catch (RuntimeException ex) {
            Notification.show(ex.getMessage());
        }
    ...
    private String format(Booking booking) {
        return """
                Reference: %s
                Film: %s
                Date: %s
                ...
                booking.getStatus(),
                cancellationService.canCancel(booking) ? "Yes" : "No",
                booking.getCancellationCharge() == null ? "0.00" : booking.getCancellationCharge());
    }
