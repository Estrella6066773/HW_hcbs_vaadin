...
| 功能 | 路由 | 说明 |
| --- | --- | --- |
| **主页** | `/` | 海报墙展示全部影片；支持按片名、类型或简介搜索；点击卡片进入详情页。 |
| **影片详情** | `/film/{id}` | 单部影片的简介、演员、评分与即将上映场次。 |
| **场次列表** | `/listings` | 按城市、影院、日期、片名筛选场次；表格展示片名、简介、演员、类型、分级、影院、银幕、日期、起止时间、时段与余座。 |
| **订票** | `/booking` | 在允许的时间范围内选择场次与座位区域（下厅 / 上厅），选定空闲座位后确认，生成带唯一参考号的收据。 |
| **取消订票** | `/cancellation` | 按参考号查询订单，查看可否取消及手续费，符合条件时完成取消。 |

**未实现**（作业范围外）：管理员 / 经理界面、登录与权限、支付。
...
**端口规则：** 优先使用 **8080**；若已被占用，则按哈希依次尝试最多 **20** 个端口（范围 **8081–8280**，种子含项目标识、工作目录与尝试序号）。20 次均失败则**退出进程**。可用 `-Dserver.port=9090` 或环境变量 `SERVER_PORT=9090` 手动指定。
